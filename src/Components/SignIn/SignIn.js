...
import { CartProducts, ResellerMail, UserData } from '../../App';
...
const SignIn = () => {
    ...
    const handleGoogleSignIn = () => {
        googleSignIn()
        .then(gotLoginData => {
            fetch(`https://ebandhonnewserver-production-89c9.up.railway.app/get-user-data/id?id=${gotLoginData.uid}`)
            ...
            .then(data => {
                if (data.uid) {
                    ...
                    fetch('https://ebandhonnewserver-production-89c9.up.railway.app/add-user-data', {
                        method: 'POST',
                    ...
                    .then(data => {
                        if(data.uid){
                            ...
                            .then(res => res.json())
                            .then(result => {
                                const isReseller = result.find(mail => mail.email === data.email)
                                if(isReseller) {
                                    setReseller(true)
                                    setSignedInUser(gotLoginData)
                                    setLoginData(gotLoginData)
            ...
            if(email) {
                if(password) {
                    // if(imageLink){
                        ...
                        fetch('https://ebandhonnewserver-production-89c9.up.railway.app/register-new-user', {
                            method: 'POST',
                            headers: {'Content-Type': 'application/json'},
    ...
    const emailPassLogin = (e) => {
        ...
        if(email) {
            if(password) {
                ...
                fetch('https://ebandhonnewserver-production-89c9.up.railway.app/email-pass-login', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json'},
                    body: JSON.stringify(data)
                })
                ...
                .then(data => {
                    if(data.error) {
                        ...
                        .then(res => res.json())
                        .then(result => {
                            const isReseller = result.find(mail => mail.email === data.data.email)
                            if(isReseller) {
                                setReseller(true)
