...
const UploadPage  = () => {
  ...
  const onSubmit = data => {
    if(selectedBrand === 'Select Brand' || selectedBrand === '') {
      ...
      fetch(`https://ebandhonnewserver-production-89c9.up.railway.app/add-product/id?id=${category._id}`, {
          method:'PATCH',
      ...
      }
      {
        adminSecret.status === true && <div className="mform container">
          ...
          <form className="form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="upload_reseller_price product_field">
              <label htmlFor="upload_reseller_price">Reseller Price</label>
              <input {...register("resellerPrice")} id="upload_reseller_price" type="number" placeholder="Reseller Price..." required/>
            </div>
            {
