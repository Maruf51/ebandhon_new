...
const ProfilePage = () => {
    useEffect(() => {
        document.title = "Profile | E-Bandhon"
      }, [])
    ...
    useEffect(() => {
        if(loginData.isSignedIn) {
            fetch(`https://ebandhonnewserver-production-89c9.up.railway.app/get-profile-data/id?id=${loginData.uid}`)
            .then(res => res.json())
            .then(data => {
                setLoginData(data)
            })
                                ...
                                <div className='purchased_items'>
                                    {
                                        loginData.boughtProducts ? 
                                            loginData.boughtProducts.map((product, index) => <PurchasedProducts products={product} index={index} />)
                                        : 
